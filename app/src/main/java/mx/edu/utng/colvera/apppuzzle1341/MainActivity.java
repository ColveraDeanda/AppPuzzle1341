 ...
 */
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        if (tipo == 1 && motionEvent.getAction() == MotionEvent.ACTION_UP) {
            banPar = !banPar;
            ...
            switch (tipo) {
                ...
                case 3:
                    if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                        x1 = (int) motionEvent.getX();//obteniendo las cooredenadas
                        y1 = (int) motionEvent.getY();//del inicio del touch
                    }
                    if (motionEvent.getAction() == MotionEvent.ACTION_UP) {
                        x2 = (int) motionEvent.getX();//obteniendo las cooredenadas
                        y2 = (int) motionEvent.getY();//del fin del touch
                        dx = Math.abs(x2 - x1);
                        dy = Math.abs(y2 - y1);
                        if (dx > dy) { //mov. horizontal
                            if (x2 > x1) {//hacia derecha
                                dxy = 2;
